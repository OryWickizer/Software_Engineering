# CSC-510-001-Group22-Fall2025
Course Submissions by Group 22 for CSC 510 (001): Software Engineering
This repo contains two major subprojects: `proj1` and `proj2`.

# Group Members
- Ory Wickizer
- Stephen Liu
- Atharva Waingankar
- Yash Dive

# Project Structure
- **`proj1/`**: Contains all files related to Project 1.
- **`proj2/`**: 
    - Includes frontend (React + Vite) and backend (Node.js) code.
    - For detailed instructions on setting up and running the projects, refer to the respective `proj2/README.md` file.

# Getting Started

1. Clone the repository:
   ```bash
   git clone <repository-url>
   cd Software_Engineering/proj2
   ```
2. Run the Client:
   ```bash
   cd client
   npm install
   npm run dev
   ```
3. Run the Server:
   ```bash
   cd server
   npm install
   npm run dev
   ```

   - Frontend: http://localhost:5173
   - Backend: http://localhost:3000